...
final departmentProjectsProvider = FutureProvider.family<List<Map<String, dynamic>>, String>((ref, departmentId) async {
  return await ref.read(academicRepositoryProvider).getDepartmentProjects(departmentId);
});
...
class SpecializationProjectsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // In a real app, we'd get the student's department ID from their profile
    // For now, we'll use a mock
    // Mocking a department ID for demonstration
    final departmentId = "77777777-7777-7777-7777-777777777777"; 
              ...
              return SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    ...
                    return _buildProjectCard(context, project, isGlass, isArabic, index);
                  },
  ...
  Widget _buildProjectCard(BuildContext context, Map<String, dynamic> project, bool isGlass, bool isArabic, int index) {
    final title = isArabic ? (project['title_ar'] ?? project['title_en']) : project['title_en'];
    final description = isArabic ? (project['description_ar'] ?? project['description_en']) : project['description_en'];
      ...
      child: GlassContainer(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Icon(LucideIcons.calendar, size: 16, color: Colors.white38),
                const SizedBox(width: 8),
                Text(
                  DateTime.parse(project['created_at']).toLocal().toString().split(' ')[0],
                  style: GoogleFonts.shareTechMono(color: Colors.white38, fontSize: 12),
                ),
                const Spacer(),
                TextButton(
                  onPressed: () {
                    // Navigate to project details or apply
                  },
                  child: Row(
                    children: [
                      Text(
